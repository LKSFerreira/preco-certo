...
echo -e "\033[1;35m💎 GERADOR DE TOKEN PREMIUM (VIA PROCESSOR)\033[0m"

# 1. Configurações
PLANO=${1:-trial}
COMPOSE_FILE=".docker/compose.yaml"

# 2. Executa o script Python dentro do container processor
# O container 'processor' já tem acesso direto ao banco e as dependências python.
echo -e "\033[0;90mExecutando comando no container...\033[0m"
RESPOSTA=$(docker compose -f "$COMPOSE_FILE" exec -T processor python scripts/gerar_token.py --plano "$PLANO" 2>&1)
EXIT_CODE=$?
...
if [[ "$RESPOSTA" == *"relation \"tokens\" does not exist"* ]]; then
    ...
    docker compose -f "$COMPOSE_FILE" exec -T processor python scripts/init_db.py
...
if [ $EXIT_CODE -ne 0 ]; then
    echo ""
    echo -e "\033[0;33m💡 DICA: Verifique se os containers 'database' e 'processor' estão rodando.\033[0m"
