...
### 27/02: [Curadoria Responsiva (Produ��o)](./walkthrough/curadoria_responsiva.md) ??
*   **Escopo Preservado:** Ajustes restritos a responsividade (tamanho, espa�amento, limites e rolagem), sem alterar core, estilo, brilho ou efeitos.
...
*   **Fluxo de Formul�rio Aprimorado:** `ModalFormularioProduto` ganhou progress�o de foco por Enter, visibilidade de a��es com teclado aberto e acessibilidade de teclado no "Auto Preencher".
*   **Curadoria Aplicada:** `ModalTutorialUso` e `ModalTutorialFoto` tiveram remo��o dos �cones superiores e suporte melhor de rolagem vertical em alturas reduzidas.
*   **Valida��o T�cnica:** Build de produ��o validado com sucesso no fluxo Docker oficial (`docker compose -f .docker/compose.yaml run --rm app npm run build`).

### 27/02: [Refino de Formul�rio e Scanner](./walkthrough/refino_formulario_scanner.md) ??
*   **Fluxo de Foco Corrigido:** `ModalFormularioProduto` passou a avan�ar por campos pendentes com base em validade real, evitando salto para pre�o quando `tamanho` vem parcial da API.
*   **Valida��o Coerente de Tamanho:** Regras de `REGEX_UNIDADE` foram alinhadas entre foco, indica��o visual e libera��o de salvamento.
*   **Feedback de Erro no Campo:** `Tamanho` inv�lido agora exibe destaque visual e mensagem contextual com exemplos (`1L`, `500g`, `250ml`).
*   **Tutorial com Continuidade:** Fluxo do `AUTO PREENCHER` no primeiro uso foi ajustado para seguir direto para sele��o/c�mera ap�s concluir o tutorial.
...
*   **Valida��o T�cnica:** Build de produ��o validado com sucesso via Docker (`docker compose -f .docker/compose.yaml run --rm app npm run build`).
...
### 28/02: [Normaliza��o Flex�vel de Unidades](./walkthrough/normalizacao_unidades.md)
*   **Camada �nica de Normaliza��o:** Criada a fun��o `normalizarTamanho` com reuso de `UNIT_MAP`, aplicada antes da valida��o para aceitar entradas inconsistentes sem travar o fluxo.
*   **Can�nico SI + Comercial:** Padroniza��o para `L`, `mL`, `g`, `kg`, `mg`, `uni`, `cx`, `pct`, `p�`, com separa��o obrigat�ria `n�mero + espa�o + unidade`.
*   **Integra��o Ponta a Ponta:** Normaliza��o aplicada no formul�rio (`blur`/`submit`) e nos fluxos OFF/Cosmos/IA, removendo `toUpperCase` conflitante no campo `tamanho`.
