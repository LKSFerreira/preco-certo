...
export class OrquestradorPagamento {
    ...
    private obterPlanoBanco(planoId: string): PlanoBanco {
        ...
        if (!planoBanco) {
            throw new Error('Plano invalido. Aceitos: plano_cafe, plano_lanche, plano_apoiador');
        }
        return planoBanco;
    }
    ...
    async confirmarPagamento(pagamentoId: string, planoId: string): Promise<ResultadoConfirmacao> {
        ...
        if (tokenExistente) {
            return {
                ...
                mensagem: 'Token ja foi gerado para este pagamento. Use a tela de ativacao para recupera-lo.',
            };
        }
        ...
        if (this.obterGatewayAtivo() === 'mockado') {
            ...
            return {
                ...
                plano: planoBanco,
                duracao_dias: obterDuracaoPorPlano(planoBanco),
            };
        }
        ...
        if (this.obterModoConfirmacao() === 'manual') {
            return {
                ...
                mensagem: 'Pagamento em modo manual. A liberacao depende de aprovacao de suporte.',
            };
        }
    ...
    async aprovarPagamentoManual(parametros: {
        ...
        if (solicitacao.rows.length === 0) {
            throw new Error('Solicitacao manual nao encontrada para este pagamento.');
        }
        ...
        if (dadosSolicitacao.status === 'rejeitado') {
            throw new Error('Solicitacao manual rejeitada. Nao e possivel aprovar este pagamento.');
        }
