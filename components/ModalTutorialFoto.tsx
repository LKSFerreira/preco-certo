...
import { CHAVE_TUTORIAL_FOTO_VISTO } from '../hooks/useTutorialFoto';
import BotaoConfirmaComShimmer from './buttons/BotaoConfirmaComShimmer';

// ConfiguraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o da velocidade da animaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o (em milissegundos). 
const VELOCIDADE_ANIMACAO_MS = 6000;

// ConfiguraÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o do zoom que o produto dÃƒÆ’Ã‚Â¡ ao se aproximar para a foto.
...
/**
 * Tutorial visual de foto (Tela ÃƒÆ’Ã…Â¡nica).
 * Cena ilustrativa realista.
 * Ensina que o zoom deve focar no rÃƒÆ’Ã‚Â³tulo completo (da marca ao peso) dentro do visor.
 */
export const ModalTutorialFoto: React.FC<PropsTutorialFoto> = ({ aoFechar }) => {

    const [direcaoDescarte, setDirecaoDescarte] = useState(1);
    
    // Controle de Responsividade DinÃƒÆ’Ã‚Â¢mica
    ...
    useEffect(() => {
        const observer = new ResizeObserver((entries) => {
            for (let entry of entries) {
                // O contentRect.height mede o espaÃƒÆ’Ã‚Â§o interno, descontando as bordas do simulador (12px top + 12px bottom = 24px)
                const height = entry.contentRect.height;
                
                // DISPOSITIVOS MUITO PEQUENOS (Android BÃƒÆ’Ã‚Â¡sico - Inner Medido: ~616px)
                if (height <= 630) {
                ...
                // DISPOSITIVOS MÃƒÆ’Ã¢â‚¬Â°DIOS (Entre 711px e 850px - Inner Medido: entre ~687px e ~826px)
                else if (height <= 820) {
    ...
    // VariÃƒÆ’Ã‚Â¡veis de layout baseadas no tamanho
    const isMuitoPequeno = tamanhoTela === 'muito-pequeno';
    const isIphoneSE = tamanhoTela === 'iphone-se';
    // isPequeno engloba o Tela Custom e herda as fontes jÃƒÆ’Ã‚Â¡ configuradas para o iPhone SE
    ...
    // A regra anterior mantida intacta: Pequenos usam os tamanhos do MÃƒÆ’Ã‚Â©dio
    ...
    return (
        <div
            ...
            style={{
                ...
                // Ajuste proporcional isolado para o Android BÃƒÆ’Ã‚Â¡sico (Muito Pequeno) - Valores aumentados para aproveitar o espaÃƒÆ’Ã‚Â§o
        ...
        >
            ...
            {/* Wrapper Principal com Flexbox flexÃƒÆ’Ã‚Â­vel - CorreÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o pontual de padding para isMedio para evitar esmagamento do card azul */}
            <div className={`relative h-full flex flex-col justify-between z-10 px-5 max-w-md mx-auto w-full transition-all ${isMuitoPequeno ? 'py-4 pt-6' : isIphoneSE ? 'py-4 pt-4' : isPequeno ? 'py-5 pt-6' : isMedio ? 'py-5 pt-8' : 'py-8 pt-10'}`}>
                ...
                <div className={`text-center flex-none animate-fade-in flex flex-col items-center ${isMuitoPequeno ? 'mb-1' : 'mb-2'}`}>
                    ...
                    {/* Fonte aumentada de 22px para 25px no Android BÃƒÆ’Ã‚Â¡sico e para 36px (par) nos muito grandes */}
                    ...
                    {/* Fonte aumentada de 13px para 14px no Android BÃƒÆ’Ã‚Â¡sico e para 18px (par) nos muito grandes */}
                ...
                {/* ÃƒÆ’Ã‚Ârea da AnimaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o Explicativa - Padding corrigido (p-4) para isMedio para evitar sobreposiÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o interna */}
                <div className={`relative flex-1 min-h-0 w-full bg-white/10 backdrop-blur-md rounded-[2rem] border border-white/20 flex flex-col items-center justify-center shadow-2xl pointer-events-none transition-all ${isMuitoPequeno ? 'p-4 mt-2' : isPequeno ? 'p-5 mt-3' : isMedio ? 'p-4 mt-3' : 'p-6 mt-4'}`}>
                    ...
                    {/* Texto de InstruÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o Animado - Padding superior (pt-3) adicionado isoladamente para o iPhone SE */}
                    <div className={`text-center flex-none flex flex-col items-center justify-center transition-all w-full px-4 ${isMuitoPequeno ? 'mb-2' : isIphoneSE ? 'pt-3 mb-2' : isCompacto ? 'mb-3' : 'mb-4'}`}>
                        {/* Fonte aumentada de 13px para 16px no Android BÃƒÆ’Ã‚Â¡sico e aumentada para 18px (par) nos muito grandes */}
                        <p className={`text-white font-bold animate-text-instruction-sync leading-tight w-full ${isMuitoPequeno ? 'text-[16px]' : isPequeno ? 'text-[16px]' : isMedio ? 'text-sm' : isMuitoGrande ? 'text-[18px]' : 'text-base'}`}>
                            <span className="text-green-400 font-bold block uppercase tracking-wide">Enquadre o rÃƒÆ’Ã‚Â³tulo frontal</span>
                            <span className="block mt-0.5 opacity-90">Tire uma foto nÃƒÆ’Ã‚Â­tida!</span>
                        </p>
                    </div>

                    {/* Visor da CÃƒÆ’Ã‚Â¢mera Animado - Tamanho ajustado (max-w-[200px]) especificamente para isMedio nÃƒÆ’Ã‚Â£o esmagar */}
                    <div className={`relative flex-none w-full aspect-[4/5] flex items-center justify-center rounded-2xl bg-black/40 overflow-hidden shadow-inner backdrop-blur-sm border border-white/10 transition-all ${isMuitoPequeno ? 'max-w-[190px]' : isPequeno ? 'max-w-[210px]' : isMedio ? 'max-w-[200px]' : 'max-w-[240px]'}`}>
                        ...
                        <div className="animate-product-focus-sync relative flex items-center justify-center z-10 w-full h-full">
                            <div className="relative w-[180px] h-[245px] bg-gradient-to-br from-red-600 to-red-800 rounded-md shadow-[0_10px_30px_rgba(0,0,0,0.5)] overflow-hidden border border-red-500">
                                ...
                                <div className="absolute top-2 left-2 bg-[#FFD700] px-1.5 py-1 rounded-sm shadow-sm"><p className="text-[#0d47a1] text-[6px] font-black leading-none text-center">EMBALAGEM<br />ECONÃƒÆ’Ã¢â‚¬ÂMICA</p></div>
                                <div className="absolute top-0 right-4 bg-white border border-black px-1.5 py-1 rounded-b-lg shadow-sm"><p className="text-black text-[5px] font-bold leading-none text-center">ALTO EM<br />AÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã…Â¡CAR</p></div>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-green-500/20 backdrop-blur-sm animate-photo-success-sync z-40 opacity-0">
                            <div className="flex flex-col items-center">
                                {/* CÃƒÆ’Ã‚Â­rculo aumentado de 48px para 56px no Android BÃƒÆ’Ã‚Â¡sico */}
                    ...
                    <div className={`flex-none flex flex-col w-full animate-tips-fade-in transition-all ${isMuitoPequeno ? 'mt-3 gap-1.5' : isIphoneSE ? 'mt-3 gap-2 pb-3' : isCompacto ? 'mt-5 gap-2' : 'mt-8 gap-3'}`}>
                        <div className="flex items-center gap-2">
                            {/* ÃƒÆ’Ã‚Âcone e fonte aumentados no Android BÃƒÆ’Ã‚Â¡sico */}
                        ...
                        <div className="flex items-center gap-2">
                            <svg className={`text-green-400 shrink-0 ${isMuitoPequeno ? 'w-[18px] h-[18px]' : 'w-5 h-5'}`} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                            {/* Fonte ajustada para 18px (par) nos muito grandes */}
                            <span className={`text-white font-medium leading-tight ${isMuitoPequeno ? 'text-[16px]' : isPequeno ? 'text-[15px]' : isMedio ? 'text-sm' : isMuitoGrande ? 'text-[18px]' : 'text-base'}`}>Garanta que o texto esteja legÃƒÆ’Ã‚Â­vel</span>
                        </div>
                    </div>
                ...
                <div className={`flex-none transition-all ${isMuitoPequeno ? 'mt-3 mb-2' : 'mt-6 mb-4'}`}>
                    <BotaoConfirmaComShimmer
                        ...
                        texto="Entendi, abrir camera!"
                        icone={'\u{1F4F8}'}
                        compacto={isMuitoPequeno || isCompacto}
                    />
                </div>
