...
const ModalContato: React.FC<PropsModalContato> = ({ aoFechar }) => {
  ...
  const modoTeclado = tecladoAtivo;
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-[2.5rem] shadow-[0_25px_60px_rgba(0,0,0,0.4)] w-full max-w-sm relative flex flex-col max-h-[calc(100dvh-0.75rem)] min-h-0 overflow-hidden"
      >
        ...
        {/* Form Body com campos rolaveis e CTA protegido contra compressao */}
        <form onSubmit={lidarSubmit} className={`flex-1 min-h-0 overscroll-contain flex flex-col ${modoCompacto ? 'gap-3 p-4' : 'gap-5 p-6'} ${modoTeclado ? 'pb-[calc(env(safe-area-inset-bottom)+1.25rem)]' : 'pb-[calc(env(safe-area-inset-bottom)+1rem)]'}`}>
          <div className={`flex-1 min-h-0 overflow-y-auto overscroll-contain ${modoCompacto ? 'space-y-3' : 'space-y-4'}`}>
            <div>
          ...
          <div className={`shrink-0 ${modoTeclado ? 'pt-2' : 'pt-1'}`}>
            <BotaoConfirmaComShimmer
              botaoRef={refBotaoEnviar}
              tipo="submit"
